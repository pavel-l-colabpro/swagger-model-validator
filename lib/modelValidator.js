...
function validateType(name, value, field, models, allowBlankTargets, disallowExtraProperties) {
    if (value === null) {
      if (!field.nullable && !field['x-nullable']) {
        return new Error(name + ' (' + wrapNull(value) + ') can not be null');
      } else {
        return null;
      }
    }
    ...
    if(!expectedType) {
        if(models && field.$ref) {
            ...
            return validate(name, value, models[fieldRef1], models, allowBlankTargets, disallowExtraProperties);
        } else {
            return null;
        }
    } else {
